...
data class MadridTransitSnapshot(
    ...
    val storedAtEpochMillis: Long = 0L,
) {
...
class MadridTransitSnapshotStore(context: Context) {
    ...
    fun saveSnapshot(snapshot: MadridTransitSnapshot) {
        val snapshotToSave = if (snapshot.storedAtEpochMillis > 0L) {
            snapshot
        } else {
            snapshot.copy(storedAtEpochMillis = System.currentTimeMillis())
        }
        val editor = preferences.edit()
        if (snapshotToSave.updatedAt.isBlank() || snapshotToSave.items.isEmpty()) {
            editor.remove(KEY_SNAPSHOT)
        } else {
            editor.putString(KEY_SNAPSHOT, MadridTransitSnapshotCodec.encode(snapshotToSave))
        }
        editor.apply()
    }
}
...
object MadridTransitSnapshotCodec {
    fun encode(snapshot: MadridTransitSnapshot): String {
        val header = listOf(
            SNAPSHOT_VERSION,
            snapshot.updatedAt.encoded(),
            snapshot.storedAtEpochMillis.toString(),
    ...
    fun decode(raw: String?): MadridTransitSnapshot? {
        ...
        val storedAtEpochMillis = headerParts.getOrNull(2)?.toLongOrNull() ?: 0L
