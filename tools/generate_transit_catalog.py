...
def write_kotlin(output: Path, rows: List[CatalogRow], summaries: List[str]) -> None:
    output.parent.mkdir(parents=True, exist_ok=True)
    raw = "\n".join(row.encoded() for row in rows)
    chunks = chunk_text(raw, CHUNK_SIZE)
    generated_at = time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime())
    summary_text = "\n".join(f" * - {summary}" for summary in summaries)
    chunk_literals = ",\n".join(textwrap.indent(kotlin_triple_string(chunk), "        ") for chunk in chunks)
...
object MadridGeneratedTransitCatalog {{
    ...
    fun optionById(id: String): MadridTransitOption? {{
        ...
        return ROW_CHUNKS.asSequence()
            .flatMap {{ chunk -> chunk.lineSequence() }}
            .map {{ line -> line.trim() }}
            .filter {{ line -> line.isNotBlank() }}
            .firstNotNullOfOrNull {{ line ->
                if (line.generatedOptionId() == id) line.toTransitOption() else null
            }}
    }}
    ...
    private val ROW_CHUNKS: Array<String> = arrayOf(
{chunk_literals}
    )
}}
""",
